...
- OpenClaw sends byte-identical history across turns (verified by diffing consecutive Responses request bodies), so conversation history is reusable. What limits the reported `cacheRead` is the provider's breakpoint placement: OpenAI documents message-end breakpoints for GPT-5.6 and later on the Platform API, while the ChatGPT-backed Responses route was observed to report hits in 1,024-token steps. See [OpenAI live expectations](#openai-live-expectations) below.
...
- Expect `cacheRead` to lag the full prompt by up to one breakpoint interval rather than tracking it exactly: repeated-turn reuse advances in provider-sized steps, not Anthropic-style moving full-history reuse.
...
Why the assertions differ: Anthropic exposes explicit cache breakpoints and moving conversation-history reuse, while OpenAI reports the last matched breakpoint, which can sit up to one interval short of the full prompt. Comparing the two providers against a single cross-provider percentage threshold produces false regressions.
...
- Every reported `cacheRead` was congruent to 512 modulo 1,024 (`32256`, `33280`, `34304`, `35328`), so this route places implicit breakpoints every 1,024 tokens. A session whose prompt grew from `32658` to `33098` tokens across four turns kept reporting `32256` until the prompt crossed the next interval; a 60-second pause between turns did not change this.
- Consecutive request bodies were byte-identical up to the newly appended items, including replayed runtime-context carriers, so the shortfall is breakpoint granularity, not history rewriting.
...
- **Low OpenAI `cacheRead`**: verify the stable prefix is at the front, the repeated prefix is at least 1024 tokens, and the same `prompt_cache_key` is reused for turns that should share a cache. A one-turn drop that recovers on the next request usually means the volatile system suffix changed; enable cache tracing and look for `systemPromptSuffix` in the `[prompt-cache]` warning. A steady shortfall of less than one breakpoint interval is provider granularity, not a prefix break.
